...
class Conversation:
    ...
    async def on_text(self, chat_id: int, user_id: int, text: str) -> bool:
        ...
        if seconds > MAX_SPEECH_SECONDS:
            await self.send(chat_id, (
                f"Это примерно {seconds:.0f} секунд речи, а ролик не длиннее"
                f" {MAX_SPEECH_SECONDS:.0f}. Сократите на"
                f" {seconds - MAX_SPEECH_SECONDS:.0f} с и пришлите ещё раз."
            ), [("Отмена", "cancel")])
            return True
        ...
        if seconds < SHORT_CLIP_SECONDS - 1:
            ...
            note = ("\n\nРеплика короткая — в таких роликах модель иногда"
                    ...
                    " фразу подлиннее.")
        await self.send(chat_id, (
            "Принял, делаю. Обычно это меньше минуты"
            + (", рисованный чуть дольше" if state.mode == "toon" else "")
            + ".\nПришлю сюда, как будет готово." + note
        ), [])
        return True
    ...
    def waiting_for(self, user_id: int) -> str:
        ...
        Нужно чат-части: по этому она понимает, ей ли отдавать вложение
        или это запись для окна мини-аппа.
        """
    ...
    async def _ask(self, chat_id: int, state: State) -> None:
        ...
        if state.step == STEP_TEXT:
            ...
            await self.send(chat_id, (
                ...
                "Напишите текст сообщением. Влезает примерно 35 слов —"
                " это 14 секунд речи."
            ), cancel)
            return
