...
class Conversation:
    ...
    async def begin(self, chat_id: int, user_id: int, greet: bool = True) -> None:
        ...
        # Длина текста здесь не про красоту. Ширину клавиатуры мессенджер
        # берёт от пузыря сообщения, а пузырь — от текста: под коротким
        # «С чего начнём?» кнопки жмутся в узкую колонку и «Рисованный
        # аватар» обрезается на полуслове. Повторное меню поэтому не
        # короче приветствия, хотя по смыслу могло бы быть в три слова.
        text = (
            "Привет! Сделаю говорящего аватара — он скажет вашим голосом то,"
            " что вы напишете.\n\n"
            "Всё прямо здесь, в переписке: я спрошу по одному шагу."
            if greet else
            "С чего начнём? Выберите, какого аватара делаем — и я проведу"
            " по шагам."
        )
