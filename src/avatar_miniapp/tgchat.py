...
class TelegramSide:
    ...
    async def deliver(self, user_id: int, video: Path, caption: str, feedback_key: str,
        ...
        try:
            await self.bot.send_message(
                # Текст длиннее, чем нужно по смыслу, намеренно: ширину
                # клавиатуры Telegram берёт от пузыря сообщения, а пузырь —
                # от текста. Под коротким вопросом кнопки сжимаются в
                # узкую колонку.
                user_id,
                "Как получилось? Оцените ролик — по этим кнопкам мы и понимаем, что чинить в первую очередь.",
