...
@pytest.mark.asyncio
async def test_new_voice_appears_without_restart(settings):
    """Положить wav и перезапустить — ровно та мелочь, из-за которой потом
    полчаса ищут, почему файл лежит, а список пуст."""
    jobs = JobManager(settings.jobs_dir, _quick, concurrency=1)
    app = build_app(settings, jobs, lambda: [
        {"id": p.stem, "title": p.stem, "note": ""}
        for p in sorted(settings.voices_dir.glob("*.wav"))
    ])
    client = TestClient(TestServer(app))
    await client.start_server()
    try:
        first = await (await client.get("/api/voices", headers=headers())).json()
        assert first["voices"] == []
        (settings.voices_dir / "anya.wav").write_bytes(b"RIFF")
        again = await (await client.get("/api/voices", headers=headers())).json()
        assert [v["id"] for v in again["voices"]] == ["anya"]
    finally:
        await client.close()
